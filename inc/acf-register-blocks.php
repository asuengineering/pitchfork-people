 ...
 */
if ( ! function_exists( 'starter_plugin_custom_category' ) ) {
	/**
	 ...
	 */
	function starter_plugin_custom_category( $categories, $editor_context ) {
		return array_merge(
			$categories,
			array(
				array(
					'slug'  => 'icenine-portfolio',
					'title' => __( 'Pitchfork Blocks', 'icenine-portfolio' ),
				),
			)
		);
	}
}
add_filter( 'block_categories_all', 'starter_plugin_custom_category', 10, 2 );
 ...
 */
function starter_plugin_acf_blocks_init() {
	...
	if ( function_exists( 'acf_register_block_type' ) ) {
		...
		foreach ( $block_includes as $folder ) {
			require_once STARTER_PLUGIN_BASE_PATH . '/acf-block-templates' . $folder . '/register.php';
		}
	}
}
add_action( 'acf/init', 'starter_plugin_acf_blocks_init' );
